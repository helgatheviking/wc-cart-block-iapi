...
const { state, actions } = store( 'woocommerce/cart', {
	...
	actions: {
			...
			if ( quantity > 0 ) {
				try {
					const response = yield fetch(
						`/wp-json/wc/store/v1/cart/update-item?key=${ cartItem.key }&quantity=${ quantity }`,
						{
							method: 'POST',
							headers: {
								Nonce: state?.nonce || '',
		...
		*removeItem( e ) {
			...
			try {
				const response = yield fetch(
					`/wp-json/wc/store/v1/cart/remove-item?key=${ cartItem.key }`,
					{
						method: 'POST',
						headers: {
							Nonce: state?.nonce || '',
...
const unsubscribe = subscribe( () => {
	...
	if ( ! cartData || cartData.itemsCount === undefined || ! cartData.items )
		return;
